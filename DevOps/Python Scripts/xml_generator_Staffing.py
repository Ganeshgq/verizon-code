...
c1 = SubElement(child1, 'listener', classname="com.verizon.cao.selenium.testcase.Driver")
c2 = SubElement(child1, 'parameter', name="browser", value="Firefox")
for test in test_names:
    child = SubElement(root, "test", name=test)
    node1 = SubElement(child, "parameter", name="testCaseId", value=test)
    node2 = SubElement(child, "classes")
    if test_labels[test] != []:
        node3 = SubElement(node2, 'class', name="com.verizon.cao.selenium.modules."+test_labels[test][0])
    else:
        print ("Found a test case "+test+" without label in JIRA. Please add class name as label.")
        sys.exit(1)
